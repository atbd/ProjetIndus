function [ s ] = iTFD_gen( t, S, Tmoy, Nfft)
...
f = linspace(-0.5,0.5,Nfft)/Tmoy; %ligne f, fréquences réparties uniformément.
delta = [0;diff(f)']; 
...
w = w(:,ones(size(f))); % duplication de la colonne

%% Création de la table des coefficients de Fourier
f = f(ones(size(t)),:); % duplication de la ligne pour créer la table des coefficients de Fourier
W = exp(w.*f);
